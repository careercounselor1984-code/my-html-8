(()=>{
  ...
  function categoryPatch(){
    ...
    let midOpenRows=[],smeRows=[],midLoaded=false,midBusy=false,smeBusy=false;
    ...
    async function searchSme(){
      ...
      try{const d=await call(GENERAL_API,{action:'recommend_jobs',keywords:[q],regionCode:REGION_CODE[$('smeSido').value]||'',limit:20,perKeyword:20,includeDetails:true});smeRows=(d.jobs||[]).map(normalizeGeneral);const ids=new Set(smeRows.map(x=>x.job_key));jobs=jobs.filter(j=>j.source!=='WORK24_GENERAL'||ids.has(j.job_key)).filter(j=>j.source!=='WORK24_GENERAL').concat(smeRows);renderSme()}catch(e){$('smeList').innerHTML='<div class="warn"><b>고용24 중소기업 검색 실패</b><br>'+esc(e.message)+'</div>'}finally{smeBusy=false}
    }
    function renderSme(){const el=$('smeList');$('smeCount').textContent=smeRows.length+'건';el.innerHTML=smeRows.length?smeRows.map(j=>jobItem(j,'sme')).join(''):'<div class="catEmpty">검색된 민간 중소기업 공고가 없습니다.</div>';bindAnalyze(el)}
