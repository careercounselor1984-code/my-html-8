(()=>{
  ...
  function patch(){
    ...
    function decorate(item){if(!item||item.querySelector('.v180AnalysisTools'))return;const j=jobFor(item);if(!j)return;const pub=publicJob(j),corp=corporateJob(j);if(!pub&&!corp)return;const actions=item.querySelector('.v166Actions');if(!actions)return;const box=document.createElement('div');box.className='v180AnalysisTools';const buttons=pub?[['institution','기관분석'],['role','직무분석'],['ncs','NCS 보기']]:[['company','기업분석'],['role','직무분석']];box.innerHTML='<span class="v180AnalysisLabel">추가분석</span>'+buttons.map(([m,l])=>`<button type="button" class="v180AnalysisBtn" data-analysis-mode="${m}">${l} ↗</button>`).join('');actions.appendChild(box)}
