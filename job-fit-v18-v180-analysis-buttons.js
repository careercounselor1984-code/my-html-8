(()=>{
  ...
  function patch(){
    ...
    function roleText(j){return text(j.job_detail||j.jobContent||j.description||j.recruit_field||j.jobsNm||(j.roles||[]).join(' ')||(j.ncs_names||[]).join(' ')||j.title).slice(0,420)}
    ...
    function analysisUrl(j,mode){const p=new URLSearchParams({mode,source:text(j.source),jobKey:text(j.job_key),company:text(j.company||j.organization),title:text(j.title),region:region(j),companyClass:text(j.companyClass||j.company_class),jobCont:roleText(j),url:text(j.source_url||j.official_url||j.url)});return 'job-fit-analysis.html?'+p.toString()}
    function decorate(item){if(!item||item.querySelector('.v180AnalysisTools'))return;const j=jobFor(item);if(!j)return;const pub=publicJob(j),corp=corporateJob(j);if(!pub&&!corp)return;const actions=item.querySelector('.v166Actions');if(!actions)return;const box=document.createElement('div');box.className='v180AnalysisTools';let buttons=pub?[['institution','기관분석'],['role','직무분석'],['ncs','NCS 보기']]:[['company','기업분석'],['role','직무분석']];box.innerHTML='<span class="v180AnalysisLabel">추가분석</span>'+buttons.map(([m,l])=>`<button type="button" class="v180AnalysisBtn" data-analysis-mode="${m}">${l} ↗</button>`).join('');actions.appendChild(box)}
    document.querySelectorAll('.v166Item').forEach(decorate);
    document.addEventListener('click',e=>{const b=e.target.closest?.('.v180AnalysisBtn');if(b){e.preventDefault();e.stopPropagation();const item=b.closest('.v166Item'),j=jobFor(item);if(j)window.open(analysisUrl(j,b.dataset.analysisMode||'role'),'_blank','noopener');return}const s=e.target.closest?.('.v166Summary');if(s){const item=s.closest('.v166Item');setTimeout(()=>decorate(item),40);setTimeout(()=>decorate(item),300)}},true);
